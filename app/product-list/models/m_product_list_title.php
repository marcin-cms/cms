	...
	class Product_list_title {
		...
		function display_block($id, $type = 'wysiwyg'){
			...
			if($this->SK->Auth->checkLoginStatus()){
				...
				$edit_type = '<a class="sk_edit_type label label-inverse" href="'. SITE_PATH.'app/product-list/edit_title.php?id='.$id.'&type='.
				$type. '">'.$type . '</a>';
				$edit_link = '<a class="sk_edit_link" href="'.SITE_PATH. 'app/product-list/edit_title.php?id='.$id.'&type='.
				$type . '">Edytuj blok</a>';
				$edit_end = '</div>';
		...
		function load_block($id){
			if($stmt = $this->SK->Database->prepare("SELECT title_product From products WHERE id = ?")){
		...
		function update_block($id,$content){
				if($stmt = $this->SK->Database->prepare("UPDATE products SET title_product = ? WHERE id = ?")){
