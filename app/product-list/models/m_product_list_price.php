	...
	class Product_list_price {
		...
		function display_block($id, $type = 'wysiwyg'){
			...
			if($this->SK->Auth->checkLoginStatus()){
				...
				$edit_type = '<a class="sk_edit_type label label-inverse" href="'. SITE_PATH.'app/product-list/edit_price.php?id='.$id.'&type='.
				$type. '">'.$type . '</a>';
				$edit_link = '<a class="sk_edit_link" href="'.SITE_PATH. 'app/product-list/edit_price.php?id='.$id.'&type='.
				$type . '">Edytuj blok</a>';
				$edit_end = '</div>';
		...
		function load_block($id){
			if($stmt = $this->SK->Database->prepare("SELECT price From products WHERE id = ?")){
		...
		function update_block($id,$content){
				if($stmt = $this->SK->Database->prepare("UPDATE products SET price = ? WHERE id = ?")){
